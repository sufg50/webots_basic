"""
robot_cat_auto_02.pyを改造して、マニュアル運転と自動運転を切り替えられるようにしました。
【追加機能】
- キーボードの矢印キーで、スピードとハンドルの角度を手動で変更できるようにしました。
- [A]キーを押すと自動運転モード、[S]キーを押すとマニュアル運転モードになります。
"""
...
class RobotCar():
    ...
    def __init__(self):  
        ...
        self.cmd_speed          = self.SPEED # 速度指令値
    ...
    def run1(self): 
        step = -1
        self.keyboard.enable(self.TIME_STEP) # キーボー入力の有効化
...
def main():  
    ...
    robot_car.run1()        # if文による障害物回避,手動操作の切り替え
